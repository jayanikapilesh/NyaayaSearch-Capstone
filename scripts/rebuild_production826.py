...
print(f"Total labeled query->section pairs (TRUE production 826-query set, batches 1+2 only): {len(queries_and_targets)}")
...
df.to_csv("../data/eval/classifier_training_data_production826.csv", index=False)
print(f"\nBuilt {len(df)} labeled examples, saved to classifier_training_data_production826.csv")
